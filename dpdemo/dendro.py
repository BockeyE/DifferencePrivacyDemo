...
# // add an internal edge, O(1)
    def		addEdge(self,const int, const int, const short int);
    return
# // returns the ith edge of edgelist, O(1)
    def   getEdge(self,const int);
    return
# // returns a uniformly random internal edge, O(1)
    def    getRandomEdge(self,);
         return
# // writes edgelist to terminal
    def 	printEdgeList(self,);
         return
# // swaps two edges, O(1)
    def 		swapEdges(self, int,  int,   int,  int,  int,   int):
        return
...
int         countEdgeBelow(elementd*);											//count edges in graph in the subtree
void        updateNoisyPr(elementd*, double, Laplace*, bool);					//update connection probabilities with laplacian noise in the subtree
...
bool			monteCarloMove(double&, bool&, double);			// make single MCMC move
...
int dendro::countEdgeBelow(elementd* curr){
...
return curr->e+ countEdgeBelow(curr->L)+ countEdgeBelow(curr->R);
}
}
return -1;
}
...
int e = curr->e;
int nL_nR = curr->L->n*curr->R->n;
int nL= curr->L->n;
int nR = curr->R->n;
double scale = sen/(nL_nR*epsilon);			//compute Laplacian noise scale if updating only this internal node
int total = (nL+nR)*(nL+nR-1)/2.0;			//the number of total possible edges

double community_scale = 1.0/(epsilon*total);	//compute Laplacian noise scale if using Erdős–Rényi random graph model
...
int num_of_edges_below = countEdgeBelow(curr);
...
curr->noisy_p =  (e+noise)/(double)nL_nR;
...
updateNoisyPr(root, epsilon, lap,flag_control);		//add noise to the connection probs in the entire dendrogram
...
p_array[i] = internal[i].p;
int nLnR =   internal[i].L->n*internal[i].R->n;
cout<<"p:"<<p_array[i]<<"\t noisy p:"<<new_noise[i]<<"\tnLnR:"<<nLnR<<endl;
...
for (int i=0; i<n; i++) {			//randomly generate each link between each pair of nodes in graph according to the connection probabilities
for (int j=(i+1); j<n; j++) {
...
if (!(random_g->doesLinkExist(i,j))) { if (!(random_g->addLink(i,j))) { cout << "Error: (" << j << " " << i << ")" << endl; } }
if (!(random_g->doesLinkExist(j,i))) { if (!(random_g->addLink(j,i))) { cout << "Error: (" << j << " " << i << ")" << endl; } }
}
}
}
//	random_g->printPairs();
...
if(epsilon){
double noise = lap.get_laplacian_random_variable(sen/epsilon);
int e = commonAncestor->e;
double p = commonAncestor->p;
double noisy_p = 0;
if(e==0 or p==0){
noisy_p = noise;
}else{
...
bool dendro::monteCarloMove(double& delta, bool& ftaken, double T) {
